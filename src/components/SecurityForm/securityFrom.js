...
//-----------------------------validate-----------------------------------
  ...
  validate = () => {
    ...
    if (this.state.receivingSecuritySupervisor.length < 3 ){
      isError = true;
      errors.receivingSecuritySupervisorError = "Full name should be provided";
    }
    else {
      errors.receivingSecuritySupervisorError = "";
    }

    if (isNaN(this.state.guardRegularRate) || this.state.guardRegularRate.replace(/\s/g, "").length == 0){
      isError = true;
      errors.guardRegularRateError = '# of hrs for guard regular rate should be provided';
    }
    else {
    ...
    if (isNaN(this.state.guardOTRate) || this.state.guardOTRate.replace(/\s/g, "").length == 0){
      isError = true;
      errors.guardOTRateError = '# of hrs for guard OT rate should be provided';
    }
    else {
    ...
    if (isNaN(this.state.SCSPRegularRate) || this.state.SCSPRegularRate.replace(/\s/g, "").length == 0){
      isError = true;
      errors.SCSPRegularRateError = '# of hrs for SCSP regular rate should be provided';
    }
    else {
    ...
    if (isNaN(this.state.SCSPOTRate) || this.state.SCSPOTRate.replace(/\s/g, "").length == 0){
      isError = true;
      errors.SCSPOTRateError = '# of hrs for SCSP OT rate should be provided';
    }
    else {
